...
parser.add_argument('--metric_network', type=str, default='gnn', metavar='N',
                    help='gnn')
parser.add_argument('--dataset', type=str, default='cancer', metavar='N',
                    help='cancer')
...
parser.add_argument('--feature_num', type=int, default=41, metavar='N',
...
parser.add_argument('--w_feature_num', type=int, default=37, metavar='N',
...
parser.add_argument('--iterations', type=int, default=600, metavar='N',
...
parser.add_argument('--batch_size', type=int, default=2, metavar='batch_size',
                    help='Size of batch)')
parser.add_argument('--batch_size_test', type=int, default=2, metavar='batch_size',
                    help='Size of batch)')
parser.add_argument('--batch_size_train', type=int, default=2, metavar='batch_size',
                    help='Size of batch)')
...
class Generator(data.DataLoader):
    def __init__(self, root,keys = ['T1','T2','T3']):
    ...
    def get_task_batch(self, batch_size=2, n_way=4, num_shots=10, unlabeled_extra=0, cuda=False, variable=False):
        # init
...
def compute_adj(batch_x, batches_xi):
    ...
    sex = nodes.narrow(2, 0, 1)
    sex = sex.cpu().numpy()
    age = nodes.narrow(2, 1, 1)
    age = age.cpu().numpy()
    smoke = nodes.narrow(2, 2, 1)
    smoke = smoke.cpu().numpy()
    drink = nodes.narrow(2, 3, 1)
    drink = drink.cpu().numpy()
    adj = np.ones(
    ...
    for batch_num in range(args.batch_size):
        for i in range(args.train_N_way * args.train_N_shots + 1):
            for j in range(i + 1, args.train_N_way * args.train_N_shots + 1):
                if sex[batch_num, i, 0] == sex[batch_num, j, 0]:
                    adj[batch_num, i, j, 0] -= 1
                    adj[batch_num, j, i, 0] -= 1
                if np.abs(age[batch_num, i, 0] - age[batch_num, j, 0]) <= 0.06:
                    adj[batch_num, i, j, 0] -= 1
                    adj[batch_num, j, i, 0] -= 1
                if smoke[batch_num, i, 0] == smoke[batch_num, j, 0]:
                    adj[batch_num, i, j, 0] -= 1
                    adj[batch_num, j, i, 0] -= 1
                if drink[batch_num, i, 0] == drink[batch_num, j, 0]:
                    adj[batch_num, i, j, 0] -= 1
                    adj[batch_num, j, i, 0] -= 1
    adj = 1/adj
...
def test_one_shot(args, fold,test_root, model, test_samples=66, partition='test',io_path= 'run.log'):
    io = io_utils.IOStream(io_path)

    io.cprint('\n**** TESTING BEGIN ***' )
    root = test_root
    loader = Generator(root,keys = ['T1','T2','T3'])
    [amgnn, softmax_module] = model
    amgnn.eval()
    correct = 0
    total = 0
...
if __name__ =='__main__':
    ...
    save_path = 'result1/{}'.format(name)

    if name not in os.listdir('result1/'):
    ...
    for batch_idx in range(args.iterations):
        ...
        root = '/home/weiweili/notebook/QIANLI/gnn/class3/rpc24/train.pkl'
        da = Generator(root, keys=['T1', 'T2','T3'])
        ...
        if batch_idx % args.log_interval == 0:
            ...
            display_str += '/tLoss_d_metric: {:.6f}'.format(total_loss / counter)
        ...
        if (batch_idx + 1) % args.log_interval == 0:
            ...
            test_samples = 66
            test_root = '/home/weiweili/notebook/QIANLI/gnn/class3/rpc24/test.pkl'
